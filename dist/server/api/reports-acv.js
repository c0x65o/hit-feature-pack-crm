...
export async function GET(request) {
    try {
        ...
        const result = await db
            .select({
            avg: sql `AVG(${crmDeals.amount}::numeric)`,
            count: sql `count(*)`,
            total: sql `SUM(${crmDeals.amount}::numeric)`,
        })
            .from(crmDeals)
        ...
        return NextResponse.json({ error: 'Failed to calculate ACV' }, { status: 500 });
    }
}
//# sourceMappingURL=reports-acv.js.map