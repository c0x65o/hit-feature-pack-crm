...
export async function GET(request) {
    try {
        ...
        const pipelineHealth = await Promise.all(stages.map(async (stage) => {
            const dealsResult = await db
                .select({
                count: sql `count(*)`,
                totalValue: sql `COALESCE(SUM(${crmDeals.amount}::numeric), 0)`,
            })
                .from(crmDeals)
        ...
        return NextResponse.json({ error: 'Failed to calculate pipeline health' }, { status: 500 });
    }
}
//# sourceMappingURL=reports-pipeline-health.js.map