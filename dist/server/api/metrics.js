...
export async function GET(request) {
    try {
        ...
        const [pipelineValueResult] = await db
            .select({ total: sql `coalesce(sum(${crmDeals.amount}), 0)` })
            .from(crmDeals);
        ...
        if (closedWonStageIds.length > 0) {
            ...
            const conditions = closedWonStageIds.map((id) => eq(crmDeals.pipelineStage, id));
