...
export interface ActivityParseOptions {
  ...
  openaiApiKey?: string; // Optional direct API key (for testing or global config)
}
...
export async function parseActivityText(
  ...
  if (openaiApiKey) {
    ...
    const keyRecord = await db
      ...
      .where(
        and(
          eq(crmOpenaiKeys.customerId, customerId),
          eq(crmOpenaiKeys.isActive, true)
        )
      )
      .limit(1);
 ...
 */
export async function updateOpenaiKeyUsage(
  customerId: string,
  db: any
): Promise<void> {
  try {
