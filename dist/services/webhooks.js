 ...
 */
export async function sendWebhook(event, options = {}) {
    ...
    if (webhookUrl) {
        ...
        const config = await db
            ...
            .where(and(eq(crmWebhookConfigs.customerId, customerId), eq(crmWebhookConfigs.isEnabled, true)))
            .limit(1);
        if (config.length > 0) {
            url = config[0].webhookUrl;
            secret = config[0].secret || null;
            // Check if this event type is enabled
            const events = config[0].events;
 ...
 */
async function sendWebhookAsync(url, payload, secret, retries = 3) {
    const headers = {
 ...
 */
export async function triggerDealClosedWon(dealId, dealData, options = {}) {
    await sendWebhook({
 ...
 */
export async function triggerContactCreated(contactId, contactData, options = {}) {
    await sendWebhook({
