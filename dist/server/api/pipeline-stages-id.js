    ...
    try {
        ...
        if (body.code !== undefined) {
            ...
            if (!nextCode || nextCode !== existing[0].code) {
                return NextResponse.json({ error: 'Code cannot be changed. Code is the identity of a pipeline stage.' }, { status: 400 });
            }
        }
        // Prevent clients from changing system flag via API.
        if (body.isSystem !== undefined) {
            return NextResponse.json({ error: 'isSystem cannot be changed via API' }, { status: 400 });
        }
        ...
        if (body.isClosedWon === true) {
            ...
            if (otherWon.length > 0) {
                return NextResponse.json({ error: 'A Closed Won stage already exists. Unset Closed Won on that stage first.' }, { status: 409 });
            }
        }
        if (body.isClosedLost === true) {
            ...
            if (otherLost.length > 0) {
                return NextResponse.json({ error: 'A Closed Lost stage already exists. Unset Closed Lost on that stage first.' }, { status: 409 });
            }
        }
        // Build update object
        const updateData = {
...
export async function DELETE(request) {
    try {
        ...
        if (existing[0].isSystem) {
            return NextResponse.json({ error: 'Cannot delete system pipeline stage' }, { status: 400 });
        }
...
//# sourceMappingURL=pipeline-stages-id.js.map