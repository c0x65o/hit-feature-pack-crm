...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.code === undefined || body.code === null || typeof body.code !== 'string' || !body.code.trim()) {
      return NextResponse.json(
        { error: 'Code is required (e.g. "qualified" or "closed_won")' },
        { status: 400 }
      );
    }
    ...
    if (!code) {
      return NextResponse.json(
        { error: 'Code is invalid (use letters/numbers/underscores)' },
        { status: 400 }
      );
    }
    ...
    if (body.isSystem !== undefined) {
      return NextResponse.json(
        { error: 'isSystem cannot be set via API' },
        { status: 400 }
      );
    }
    ...
    if (body.isClosedWon) {
      ...
      if (existingWon.length > 0) {
        return NextResponse.json(
          { error: 'A Closed Won stage already exists. Update the existing stage instead.' },
          { status: 409 }
        );
      }
    }
    ...
    if (body.isClosedLost) {
      ...
      if (existingLost.length > 0) {
        return NextResponse.json(
          { error: 'A Closed Lost stage already exists. Update the existing stage instead.' },
          { status: 409 }
        );
      }
    }
