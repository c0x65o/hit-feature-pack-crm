...
export async function GET(request) {
    try {
        ...
        const conditions = [];
        if (search) {
            conditions.push(or(like(crmCompanies.name, `%${search}%`), like(crmCompanies.companyEmail, `%${search}%`), like(crmCompanies.companyPhone, `%${search}%`)));
        }
        // Apply sorting
        const sortColumns = {
            id: crmCompanies.id,
        ...
        const countQuery = db.select({ count: sql `count(*)` }).from(crmCompanies);
